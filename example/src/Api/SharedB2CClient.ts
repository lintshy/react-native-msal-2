...
export class SharedB2CClient {
    ...
    constructor(b2cConfig: B2CConfiguration) {
        ...
        this.policyUrls = makePolicyUrls(
            authorityBase,
            policies,
        )
        ...
        const knownAuthorities = Object.values(
            this.policyUrls,
        )
     ...
     */
    public async signIn(
        params: B2CSignInParams,
    ): Promise<MSALHDResult> {
        try {
            console.log(
                '[Shared B2CClient signIn] trying to signIn',
            )

            // If we don't provide an authority, the PCA will use the one we passed to it when we created it
            // (the sign in sign up policy)
            const result =
                await this.pca.signInSharedAccount(params)

            console.log(
                '[Shared B2CClient signIn] acquired token',
            )
            console.log(result)
            if (!result) {
                throw new Error(
                    'Could not sign in: Result was undefined.',
                )
            }
            return result
        } catch (error: unknown) {
            if (
                ...
                error.message.includes(
                    SharedB2CClient.B2C_PASSWORD_CHANGE,
                ) &&
    ...
    public async acquireTokenSilent(
        params: B2CSilentParams,
    ): Promise<MSALHDResult> {
        const account = await this.getAccountForRoles()

        if (!account) {
            throw Error(
                'Could not find existing account for sign in sign up policy',
            )
        ...
        try {
            const result =
                (await this.pca.acquireSharedTokenSilent({
                    ...params,
                    account,
                })) as MSALHDResult
            console.log(result)
            ...
            const out = {
                message:
                    '[Shared B2C Client]- Error while silent sign in, redirecting to log in page.',
                error,
            }
    ...
    private async resetPassword(params: B2CSignInParams) {
        ...
        if (this.policyUrls.passwordReset) {
            ...
            await this.pca.acquireSharedToken({
                ...rest,
                webviewParameters,
                authority,
            })
            ...
            throw Error(
                'B2CClient missing password reset policy',
            )
        }
    }
    ...
    private async getAccountForRoles(): Promise<
        MSALHDAccount | undefined
    > {
        const { currentAccount } =
            await this.pca.getCurrentAccount()
        console.log('in getAccountForRoles')
        if (
            isEmpty(
                this.getRolesFromUser(currentAccount) &&
                    !isEmpty(userRoles),
            )
        ) {
    ...
    public async initAccountSignIn() {
        const {
            ...
            currentEvent,
            priorAccount,
        } = await this.hasAccountChanged()

        if (
            currentAccount &&
            currentEvent === 'onAccountLoaded'
        ) {
            console.log(currentAccount)
            const res = await this.acquireTokenSilent({
                scopes: b2cScopes,
            })
            console.log('out')
            return res
